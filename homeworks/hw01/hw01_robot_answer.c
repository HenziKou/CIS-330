...
void respond(int argc, char **argv)
{
	...
	{
	    if (strcasecmp(argv[i], INPUT_STR[j]) == 0)
	    {
		// If input matches regardless of character case then print corresponding response
    ...
    if (count == 0)
	// If no matches, then print last response
	printf("%s\n", RESPONSE_STR[5]);
