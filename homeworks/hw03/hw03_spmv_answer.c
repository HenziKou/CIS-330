              ...
              int *res)
{
	...
	for (int i = 0; i < row; i++)
	{
		...
		for (int j = row_ptr[i]; j < row_ptr[i + 1]; j++)
		{
			total += val[j] * vec[col_ind[j]];
		}

		// Update result matrix at position i with respective value
		res[i] += total;
	}
}
                        ...
                        int **coo_row, int **coo_col, int **coo_val)
{
	...
	// Allocate memory
	coo_row_ = (int *) malloc(sizeof(int) * nnz);
	coo_col_ = (int *) malloc(sizeof(int) * nnz);
	coo_val_ = (int *) malloc(sizeof(int) * nnz);

	for (int i = 0; i < row; i++)
	{
		for (int j = row_ptr[i]; j < row_ptr[i + 1]; j++)
		{
			// Assign row, col, and val respective values
			coo_row_[j] = i;
              ...
              int *coo_val, int *vec, int *res)
{
	...
	for (int i = 0; i < row; i++)
	{
		...
		for (int j = count; coo_row[count] == i; j++)
		{
			total += coo_val[j] * vec[coo_col[j]];
            count ++;
		}
...
/*
	...
	for (int i = 0; i < row; i++)
	{
		...
		while (coo_row[index] == i)
		{
			total += coo_val[index] * vec[coo_col[index]];
			index++;
		}
