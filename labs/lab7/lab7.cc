...
namespace Mem {
	...
	template <class T> class Elem {
		...
		// Clearly this is overkill for this lab, but you can imagine where this might be
		// useful.
		//
		static int count;
	...
	public:
		...
		friend std::ostream &
		operator<<(std::ostream &output, const IntQueue &Q);
...
std::ostream &
Mem::operator<<(std::ostream &output, const Mem::IntQueue &Q)
{
	Mem::Elem<int> node;

	for (node.setData(Q.head->getData()); node.getData() != Q.head->getData(); node.getNext())
	{
		output << node.getData() << " ";
	}
