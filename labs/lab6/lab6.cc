...
class Filter
{
	...
	// It is `virtual` so the compiler knows to always call the overriden version in
	//  our more interesting filters.
...
string Filter::Exec(string input)
{
	...
	for (int i = 0; i < input.size(); i++) {
		...
		if (isalpha(input[i])) {
			...
			// FilterLetter for the class of the object on which this method is invoked.
			// This allows us to use this same code for each filter which inherits from
...
//
// An example of inheriting from the Filter base class to implement a filter which makes
// each alphabetic character uppercase.
//
...
//
// TODO (2) Define an Encrypt filter which uses the above shift_cypher() function to encode
// its input letters.
// This class should have a constructor that takes the shift offset which will be handed to
// shift_cypher when the filter is executed.
//
